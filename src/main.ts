...
(async () => {
  try {
    ...
    const setIgnoreMissing = validate.getIgnoreMissing();
    ...
    if (setIgnoreMissing === true) {
      core.debug('Setting ignoreMissing');
      await cli.setCommits(version, {ignoreMissing: true});
    }
