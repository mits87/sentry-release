...
/**
 * Find out from input if we should ignore missing the release.
 * @returns boolean
 */
export const getIgnoreMissing = (): boolean => {
  const ignoreMissingOption = core.getInput('ignore_missing');
  if (!ignoreMissingOption) {
    return false;
  }

  const ignoreMissing = ignoreMissingOption.trim().toLowerCase();
  ...
  throw Error('ignore_missing is not a boolean');
};
